  ...
  return (
    <Panel className={className}>
      ...
      <p className="mt-3 text-sm leading-relaxed text-muted">
        ...
        The public board still shows one Make 12 cell.
      </p>
...
export function ScenarioSimPanel({
  ...
  const [returnSlugs, setReturnSlugs] = useState(() => golden.return.teams.join(", "));
  const [error, setError] = useState<string | null>(null);
  const [request, setRequest] = useState<ScenarioSimRequest | null>(golden);
  const [response, setResponse] = useState<ScenarioSimResponse | null>(() =>
    runDemoScenarioSim(golden),
  );
    ...
    <div className={cn("space-y-6", className)}>
      ...
      <Panel>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <DeskChip>desk fixture</DeskChip>
        </div>
        <h2 className="mt-3 font-display text-2xl tracking-wide">
          Scenario Sim (preview / offline)
        </h2>
        <p className="mt-2 text-sm leading-relaxed text-muted">
          Pin one to three overrides (`force_winner` + `hx_bump`), then read
          baseline vs scenario vs Δ. This desk run is a {SCENARIO_SIM_DEMO_LABEL}.
...
function ForceWinnerEditor({
  ...
  return (
    <div className="rounded-md bg-raised/60 p-3 shadow-[var(--shadow-border)] sm:p-4">
...
function HxBumpEditor({
  ...
  return (
    <div className="mt-5 rounded-md bg-raised/60 p-3 shadow-[var(--shadow-border)] sm:p-4">
